...
func handleVoice(c chan *discordgo.Packet) {
	...
	var oggFilePath string
	for p := range c {
		file, ok := files[p.SSRC]
		if !ok {
			var err error
			oggFilePath = fmt.Sprintf("%d.ogg", p.SSRC)
			file, err = oggwriter.New(oggFilePath, 48000, 2)
			if err != nil {
...
func main() {
	...
	s.AddHandler(func(session *discordgo.Session, event *discordgo.VoiceStateUpdate) {
		fmt.Printf("VoiceState update: %+v\n", event)
		if event.ChannelID != "" {
			v, err := s.ChannelVoiceJoin(event.GuildID, event.ChannelID, true, false)
			if err != nil {
